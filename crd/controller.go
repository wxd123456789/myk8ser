...
func NewController(informer informers.CronTabInformer, kubeclient *kubernetes.Clientset) *Controller {
	...
	informer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: controller.enqueueCronTabAdd,
		UpdateFunc: func(old, new interface{}) {
			...
			controller.enqueueCronTabAdd(new)
		},
...
func (c *Controller) enqueueCronTabAdd(obj interface{}) {
	var key string
	var err error
	...
	klog.Infof("Add key[%s] to queue ", key)
...
func (c *Controller) test() {
	// list
	pods, _ := c.kubeclient.CoreV1().Pods(coreV1.NamespaceDefault).List(metav1.ListOptions{})
